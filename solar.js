...
D.createSolarAndWater = function() {
  const g = new THREE.Group(); const sx = 13, sz = -9;
  for (let r = 0; r < 3; r++) {
    for (let c = 0; c < 4; c++) {
  ...
  const pumpPanel = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 0.05), D.materials.solar); pumpPanel.position.set(-9, 0.5, -19); pumpPanel.rotation.x = -Math.PI/10; g.add(pumpPanel);
  const tank = new THREE.Mesh(new THREE.CylinderGeometry(0.8, 0.8, 1.6, 24), new THREE.MeshStandardMaterial({color:0xffffff})); tank.position.set(-9, 0.8, -19); tank.castShadow = true; g.add(tank);
  const pipeMat = new THREE.LineBasicMaterial({color:D.COLORS.water});
  const pipes = [[[-9,0.05,-19],[-9,0.05,-19]], [[-9,0.05,-19],[-2,0.05,8]], [[-9,0.05,-19],[-10,0.05,-18]], [[-9,0.05,-19],[-4,0.05,18]]];
