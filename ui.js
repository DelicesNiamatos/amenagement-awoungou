...
D.initUI = function() {
  ...
  D.canvas.addEventListener('pointermove', e => {
    ...
    if (hit) {
      ...
      D.canvas.style.cursor = 'default'; tooltip.style.display = 'none'; D.hovered = null;
    }
  });
...
D.showInfo = function(id) {
  ...
  content.innerHTML = `<h3 style="color:${hex}">● ${z.name}</h3><p><strong>Surface :</strong> ${z.area}</p><p><strong>Phase :</strong> ${z.phase}</p><p><strong>Items :</strong> ${z.items}</p><p><strong>Budget :</strong>